...
const StaffCalendar = ({ socket }) => {
  ...
  useEffect(() => {
    ...
    const getStaffDetails = async () => {
      try {
        const url = `http://54.87.167.89.nip.io:8080/db//staff/${selectedStaffEmail}`;
        const response = await axios.get(url);
  ...
  const getAppointment = async (Id) => {
    try {
      const url = `http://54.87.167.89.nip.io:8080/db/appointment/${Id}`;
      const response = await axios.get(url);
  ...
  const sendAppointmentChangeMail = async (from, to, StdReg, Apt_status) => {
    ...
    if (JSON.parse(sessionStorage.getItem("isDragged")) === true) {
      try {
        ...
        const url = `http://54.87.167.89.nip.io:8080/mail/student/update/appointment`;
